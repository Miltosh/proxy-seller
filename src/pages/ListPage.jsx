...
export const ListPage = ({ type }) => {
  ...
  return (
    <>
      <button className='button is-light mb-4' type='button' onClick={backHandler}>
        <span className='icon'>
      ...
      <div className='box'>
        {user && <h1 className='title'>{`${user.username}'s ${type}:`}</h1>}

        {isLoading && <Loader />}

        {isErrorLoading && <p className='has-text-danger'>Something went wrong</p>}

        {items &&
          items.map((item) => (
            <article className='message is-small' key={item.id}>
              <div className='message-header'>{item.title}</div>
