...
export const HomePage = () => {
  ...
  const visibleUsers = useMemo(
    () =>
      appliedQuery && users
        ? users.filter((user) => user.username.toLowerCase().includes(appliedQuery))
        : users,
    [appliedQuery, users],
  )
    ...
    <>
      ...
      <div className='box table-container'>
        ...
        {users && !users.length && !isLoading && !isErrorLoading && (
          <p>There are no users on the server</p>
        )}
        ...
        {users && !!visibleUsers.length && <UsersTable users={visibleUsers} />}
      </div>
    </>
  )
}
