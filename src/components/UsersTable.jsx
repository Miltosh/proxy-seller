...
export const UsersTable = ({ users }) => {
  ...
  const onChangeOrder = () => {
    ...
    setSearchParams(getSearchWith(searchParams, { order: newOrder }))
  }
      ...
      <tbody>
        {sortUsers.map((user) => (
          <tr key={user.id}>
            ...
            <td>
              <Link to={`${user.id}/posts`}>{`${user.username} Posts`}</Link>
            </td>
            <td>
              <Link to={`${user.id}/albums`}>{`${user.username} Albums`}</Link>
            </td>
          </tr>
        ))}
      </tbody>
